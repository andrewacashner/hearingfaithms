...
poem_exx_aux_dir = $(aux)/poem-examples
diagrams_aux_dir = $(aux)/diagrams
tables_aux_dir  = $(aux)/tables
music_aux_dir	= $(aux)/music-examples
maps_aux_dir	= $(aux)/maps
aux_dirs	= $(chapters_aux_dir) $(poem_exx_aux_dir) $(diagrams_aux_dir) \
		  $(tables_aux_dir) $(music_aux_dir) $(maps_aux_dir)

music_out_dir	= $(build)/music-examples
poem_exx_out_dir = $(build)/poem-examples
diagrams_out_dir = $(build)/diagrams
tables_out_dir  = $(build)/tables
figures_dir	= $(build)/figures
maps_out_dir	= $(build)/maps
out_dirs	= $(poem_exx_out_dir) \
		  $(diagrams_out_dir) $(tables_out_dir) \
		  $(music_out_dir) $(maps_out_dir) $(figures_dir)
...
# Other float PDFs generated by LaTeX
poem_exx_out	= $(addprefix $(build)/,$(poem_exx_in:%.tex=%.pdf))
diagrams_out	= $(addprefix $(build)/,$(diagrams_in:%.tex=%.pdf))
tables_out	= $(addprefix $(build)/,$(tables_in:%.tex=%.pdf))
...
# TARGETS and RULES
.PHONY : all ly maps texfloats count view clean reset 

# MAIN LATEX DOCUMENT
all : $(main_out)

# Compile main tex file to aux dir
$(main_aux) : $(main_in) $(tex_subfiles) $(tex_lib) $(bib) $(latexmkrc) \
    $(figures_out) $(music_out) $(maps_out) \
    $(diagrams_out) $(poem_exx_out) $(tables_out) | $(dirs)
	latexmk -r $(latexmkrc)
...
$(figures_out) : $(figures_in) | $(dirs)

build/figures/%.jpg : figures/%.jpg 
	cp -u $< $@

# LaTeX FLOATS: diagrams, poem examples, tables
texfloats : $(diagrams_out) $(poem_exx_out) $(tables_out) | $(dirs)

# diagrams
$(diagrams_out) : $(diagrams_in) $(tex_lib)

$(diagrams_out_dir)/%.pdf : $(diagrams_aux_dir)/%.pdf
	pdfcrop $< $@

$(diagrams_aux_dir)/%.pdf : diagrams/%.tex
	latexmk -outdir=$(diagrams_aux_dir) -pdfxe $<

# poem examples
$(poem_exx_out) : $(poem_exx_in) $(tex_lib)

$(poem_exx_out_dir)/%.pdf : $(poem_exx_aux_dir)/%.pdf
	pdfcrop $< $@

$(poem_exx_aux_dir)/%.pdf : poem-examples/%.tex
	latexmk -outdir=$(poem_exx_aux_dir) -pdfxe $<

# tables
$(tables_out) : $(tables_in) $(tex_lib)


$(tables_out_dir)/%.pdf : $(tables_aux_dir)/%.pdf
	pdfcrop $< $@

$(tables_aux_dir)/%.pdf : tables/%.tex
	latexmk -outdir=$(tables_aux_dir) -pdfxe $<

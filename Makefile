...
build-subdirs 	:= $(addprefix build/,odt poem-examples tables music-examples diagrams)
...
dia-pdfs	= $(dia-src:%.tex=build/%.pdf)
figures 	= $(figure-src)

floats 		= $(music-pdfs) $(dia-pdfs) $(figures)
...
aux/%.pdf : diagrams/%.tex vcbook-dia.cls
	$(dolatex) -silent $<

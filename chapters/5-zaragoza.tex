...
{Offering and Imitation (Zaragoza, 1650--1700)}
\label{ch:zaragoza}
...
\addtoindex{
    ...
    vihuela;
    villancico!liturgical function!Eucharistic devotion;
    villancico!liturgical function!sanctoral devotion
}
...
{\quoted{Let Voices Ascend to Heaven}: Linked Settings by Pablo Bruna
and Miguel Ambiela}
...
\addtoindex{
    ...
    Daroca!Santa Maríá de los Sagrados Corporales;
    Zaragoza!El Pilar
}
...
\addtoindex{
    villancico!liturgical function!Eucharistic devotion;
    villancico!dedications!al Santísimo Sacramento
}
...
\addtoindex{
    analysis!poetry!textual variants
}
...
\addtoindex{
    ...
    analysis!music!formal structure
}
...
\addtoindex{
    Ambiela, Miguel|(;
    villancico!dedications!Assumption of Mary;
...
\begin{Footnote}
    \quoted{Del Maestro Miguel Ambiela/ que fue de Lerida y despues de daroca
...
\addtoindex{villancico!liturgical function}
...
\addtoindex{analysis!music!text depiction}
...
\addtoindex{villancico!structure}
...
\begin{quoting}
    \begin{enumerate}
        ...
        \item[13.] A phoenix in the midst of burning flames, with the words,
            \foreign{Ne pereat} (Let him not perish), signifies a man who in
...
The motto \foreign{Ne pereat} described by Ricciardo (\range{no}{13}) recalls
...
built the terrestrial world with its four elements \quoted{by such order and
measure \addorig{compás} that, though they are opposed to one another, they
have peace and harmony \addorig{concordia}, and not only do they not disturb
the world, but in fact they preserve and sustain it}.%
...
{Christ as a \term{Vihuela}: José de Cáseda, \wtitle{Qué música divina}}
...
\addtoindex{
    senses!exterior;
    senses!interior;
    interpretation!poetry!theological
}
...
In the Vulgate (\scripture{1Sam 16:16}), the instrument David plays for King
...
\begin{Footnote}
    ...
    with its \quoted{harpers harping on their harps}.
\end{Footnote}
...
\quoted{relationship between Christ's suffering body, the kithara, and the
female monastic}---which would give the piece a special resonance in the
...
\foreign{cytharoedi} of today (cithara players, using the same term from Rev.
...
This instrument (\figref{fig:Bermudo-vihuela7}), Bermudo says, was particularly
...
\addtoindex{
    ...
    Guerrero, Francisco;
    intabulation;
    villancico!use with instruments;
    \PueblaConvento
}
...
\term{clarín} villancicos discussed in \chapref{ch:intro} suggests that might
have been the case).
...
meaning in copla 2, \quoted{en cada punto entera consonancia} (in each point or
...
Thus the \quoted{alta} Alto is paired with the lowest voice.
...
\addtoindex{analysis!music!counterpoint}
...
F\sh{} specified at the beginning of the next \term{compás}).
...
what was heard had to come from \quoted{the word of Christ}, and people needed

...
later.\citXXX[mention ``A que el juego'']
...
He sets the first two lines of poetry (\measurenums{1--10}) with relatively long
...
He has the chorus sing the first word, \foreign{Voces}, beginning
...
singers make a small break between \measurenums{2--3}, placing the S sound on the
last minim of \measurenum{2} or just after.
...
The word \foreign{cuenta} (\measurenum{6}) is also sung on the second minim of the
...
lo que se canta}.
For the next phrase (\foreign{que es músico el rey}) Padilla creates the effect
...
Another example is the interjection \foreign{¡O qué canto!}, which Padilla
...
the poetry.
Aside from the symbolic meanings of the two changes of meter (\measurenum{45} and
\measurenum{60}; see below), these shifts match the stress patterns implicit in
...
The internal cadences in the \term{introducción} are on G (\measurenum{19}), D
(\measurenum{27}), and G (\measurenum{44}; and the cadences at the end of the
...
to listeners.\citXXX[diss. 164 Cerone, Judd, Barnett]
...
itself.
As we have noted, the opening invocation to the \quoted{Voices of the chapel
...
the \term{respuesta}.
The first chorus sings the word \term{cuenta} on a long, offbeat note, a
blackened, dotted semibreve (that is, artificially perfected) that leaps off the
page as a visual indication to the singer to \quoted{keep count}, and makes a
clear audible symbol as well.
Padilla sets \quoted{the lightest dissonance} in \measurenums{14--19} by having the
Altus I suspend across the first minim of \measurenum{18}, making a dissonant
seventh against the Tenor's A---a light dissonance that quickly resolves to
F\sh{} and then to a cadence on G in \measurenum{19}.
On the repeat of this section, it is easy to imagine that an experienced trble
singing about \quoted{the trills of the dawn} in \measurenums{25--27} would have
improvised a trill on the suspension at this cadence.%
    \Autocite
    [See Cerone's isntructions for improvisng ornaments at cadences]
    [545--546]
    {Cerone:Melopeo}
In the \term{respuesta}, where Chorus II sings about awaiting \quoted{the
thirty-three}, Padilla writes precisely thirty-three pitches for both of the
sung vocal parts.

Where the poem says that the whole world was \quoted{waiting} for \quoted{the
sign}---that is, the Incarnate Christ---in Padilla's setting, that sign comes
with the estribillo in \measurenum{45}, where the \meterC{} sign indicates a shift
...
\term{corcheas} (modern eighth notes).
Here Padilla depicts what the words say by building up a point of imitation
\quoted{from one choir to the other} (\measurenums{45--50}) and then creating
polychoral dialogue (\measurenums{51--59}).
